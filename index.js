...
Используй вид из фото: ремонт, техника, мебель, санузел, кухня, покрытие пола, окна/вид, состояние подъезда/дома, и т.д (Ты должен распознать что есть на фото и добавить это в описание в риелторском стиле).` .;

  try {
    await ctx.reply('Генерирую описание, подожди чуть чуть... ⏳');
...
bot.action('NEW_FLOW', async (ctx) => {
  ...
  await ctx.reply(META_TEMPLATE);
  await ctx.reply('После метаданных пришли 2–12 фото и нажми кнопку ниже.', {
    reply_markup: { inline_keyboard: [[{ text: 'Готово, фото загружены ✅', callback_data: 'PHOTOS_DONE' }]] }
  });
});
