...
const META_TEMPLATE = `Пришли вводные данные.
Объект: адрес, тип (квартира/дом), площадь, комнаты, этаж/этажность, год постройки.
...
А можешь ничего не писать и я составлю объявления по фоткам`;
...
/* ─────────── НОВОЕ: общий генератор + кнопка «Сгенерировать» ─────────── */

/** Общая функция генерации описания (используется и кнопкой, и фразой) */
async function generateDescription(ctx) {
  const chatId = ctx.chat.id;
  const s = getSession(chatId);
...
- Ты распазнаешь в метаданных если пишут Адрес,Район, Площадь, Планировка, Потолки, Особенности локации, Животные, Доступно с,Цена, Контакт
...
4) Условия сделки (цена, доступно с,)
5) Блок «Для площадок»: 3–5 ключевых преимуществ в виде списка
6) Обращаться по номеру или в Telegram
...
Проанализируй фото и добавь это в описание: ремонт, техника, мебель, санузел, кухня, покрытие пола, окна/вид, состояние подъезда/дома, и т.д (Ты должен распознать что есть на фото и добавить это в описание в риелторском стиле).`;
...
bot.action('PHOTOS_DONE', async (ctx) => {
  ...
  // Было: "Теперь напиши: «Создай описание»."
  // Стало: кнопка «⚡ Сгенерировать»
  await ctx.reply('Готово, фото загружены ✅', {
    reply_markup: {
      inline_keyboard: [[{ text: '⚡ Сгенерировать', callback_data: 'GENERATE_NOW' }]]
    }
  });
});

// Обработчик кнопки «⚡ Сгенерировать»
bot.action('GENERATE_NOW', async (ctx) => {
  await ctx.answerCbQuery();
  await generateDescription(ctx);
});

// Генерация — по фразе «Создай описание» (поведение сохранено)
